...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-0">
        <h1 class="text-2xl sm:text-3xl font-bold text-white mb-6">Edit Playlist</h1>

        <form action="{{ route('playlists.update', $playlist) }}" method="POST" enctype="multipart/form-data"
            class="bg-app-gray rounded-lg p-4 sm:p-6 space-y-6"
            x-data="{ coverPreview: '{{ $playlist->cover_url ?? '' }}' }">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_public" value="0" {{ old('is_public', $playlist->is_public) == 0 ? 'checked' : '' }}
                            class="text-brand-orange focus:ring-brand-orange bg-app-darker border-gray-700">
                        <span class="ml-2 text-white">Private</span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_public" value="1" {{ old('is_public', $playlist->is_public) == 1 ? 'checked' : '' }}
                            class="text-brand-orange focus:ring-brand-orange bg-app-darker border-gray-700">
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <a href="{{ route('playlists.show', $playlist) }}" class="btn-secondary text-center">Cancel</a>
